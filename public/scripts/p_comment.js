$("#commentBtn").click(function(){
	///验证是否登录
		...
		var fd=new FormData();
		xhr.open("post","/comment/addProductComment");
		fd.append("content",document.querySelector(".comment_content").value);
		fd.append("uid",sessionStorage.getItem("id"));
		fd.append("cid",location.href.split("/")[location.href.split("/").length-1]);
		xhr.send(fd);
			...
			if(xhr.readyState==4){
				if(xhr.status==200){
					if(xhr.responseText=="ok"){
					    ...
					    setPage("/comment/getAllProductComment?page=1&cid="+location.href.split("/")[location.href.split("/").length-1]);
